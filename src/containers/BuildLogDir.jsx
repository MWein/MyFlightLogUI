  ...
  return (
    <Paper key={id} onClick={onClick} style={{ position: 'relative', width: '300px', height: '265px', margin: '7px', cursor: 'pointer' }}>
      <div style={{ width: '100%', height: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <img src={`http://${window.location.hostname}:8081/img?id=${id}&thumbnail=true`} style={{ maxWidth: '90%', maxHeight: '180px', borderRadius: '4px' }} />
      </div>
...
const BuildLogDir = ({ history }) => {
  ...
  useEffect(() => {
    ...
    if ((history.location.pathname === '/buildlog' || history.location.pathname === '/buildlog/') && projects.length === 1) {
      const path = history.location.pathname
      const slash = path[path.length - 1] === '/' ? '' : '/'
      history.push(`${url}${slash}${projects[0].id}`)
    }
