...
const LogbookPage = () => {
  ...
  const loadLogData = async () => {
    ...
    const totalTimes = {
      ...logData.reduce((acc, log) => ({
        ...
        instrument: acc.instrument + log.instrument,
        simInstrument: acc.simInstrument + log.simInstrument,
        instrumentApproaches: acc.instrumentApproaches + log.instrumentApproaches,
        crossCountry: acc.crossCountry + log.crossCountry,
        crossCountryPIC: acc.crossCountryPIC + (log.pic > 0 ? log.crossCountry : 0),
  ...
  const planePic = selectedFlight != null ? `http://${window.location.hostname}:8081/img?id=${logs.find(x => x.id === selectedFlight).ident}&thumb=true` : null
  const pictureIds = selectedFlight != null ? logs.find(x => x.id === selectedFlight).pictures: []
  const pictures = pictureIds.map(x => `http://${window.location.hostname}:8081/img?id=${x}&thumb=true`)
